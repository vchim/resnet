...
data_dir = '/Volumes/My Passport/samples1'
...
dsets = {x: datasets.ImageFolder(os.path.join(data_dir, x), data_transforms[x])
         for x in ['train','val']}
train_dset_loaders = {x: torch.utils.data.DataLoader(dsets[x], batch_size=4,
                                               shuffle=True, num_workers=4)
                for x in ['train']}
val_dset_loaders = {x: torch.utils.data.DataLoader(dsets[x], batch_size=4,
...
def train_model(model, criterion, optimizer, scheduler, num_epoch=1):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'train':
                ...
                for data in train_dset_loaders[phase]:
                    ...
                    _, preds_top3 = torch.topk(outputs.data, 3, 1, largest=True)
                    ...
                    for i in range(len(labels.data)):  # the number of labels
                        for j in range(3):  # number of topk
                            if preds_top3[i, j] == labels.data[i]:
                                running_corrects_3 = running_corrects_3 + 1
                            else:
                                j = j + 1
                        i = i + 1
            else:
            ...
            epoch_acc_3 = running_corrects_3 / dset_sizes[phase]

            print('{} Loss: {:.4f}  Top1_Accuracy: {:.4f}  Top5_Accuracy: {:.4f}'.format(
                phase, epoch_loss, epoch_acc_1, epoch_acc_3))
            # deep copy the model
            if phase == 'val' and epoch_acc_1 > best_acc_1 and epoch_acc_3 > best_acc_3:
    ...
    print("Best val Top3_Accuracy: {:4f}".format(best_acc_3))
...
model_ft = train_model(model_ft, criterion, optimizer_ft, exp_lr_scheduler, num_epoch=3)
